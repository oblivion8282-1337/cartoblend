...
class GeoScene():
	...
	def _set_prop_ui(self, key, description="", default=None, min=None, max=None):
		"""Set custom property UI hints using the Blender 5.x API"""
		try:
			ui = self.scn.id_properties_ui(key)
			if description:
	...
	def crsx(self, v):
		if isinstance(v, (int, float)):
			self.scn[SK.CRSX] = v
			self._set_prop_ui(SK.CRSX, description="Scene x origin in CRS space", default=0.0)
		else:
	...
	def crsy(self, v):
		if isinstance(v, (int, float)):
			self.scn[SK.CRSY] = v
			self._set_prop_ui(SK.CRSY, description="Scene y origin in CRS space", default=0.0)
		else:
	...
	def scale(self, v):
		self.scn[SK.SCALE] = v
		self._set_prop_ui(SK.SCALE, description="Map scale denominator", default=1, min=1)
	@scale.deleter
	def scale(self):
	...
	def zoom(self, v):
		self.scn[SK.ZOOM] = v
		self._set_prop_ui(SK.ZOOM, description="Basemap zoom level", default=1, min=0, max=25)
	@zoom.deleter
	def zoom(self):
